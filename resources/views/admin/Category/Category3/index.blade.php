...
@section('content')
    <div class="container">
        ...
        <a class="pull-left btn btn-warning" href="{{action('admin\Category3@create',[])}}"
            ...
            <table class="table table-bordered table-hover">
                ...
                @foreach($data as $v)
                    <tr>
                        ...
                        <td>{{$v->status}}</td>
                        <td>
                            <form action="{{action('admin\Category3@destroy',[$v->id])}}" method="post" role="form">
                                @method('DELETE')
                                @csrf
                                <a href="{{action('admin\Category3@edit',[$v->id])}}" class="btn btn-primary"><i
